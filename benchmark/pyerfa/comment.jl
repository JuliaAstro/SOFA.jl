...
function fmt_ns(s)
    x = parse_ns(s)
    isnan(x) && return "—"
    x < 1.0e3 && return string(round(x; sigdigits = 4), " ns")
    x < 1.0e6 && return string(round(x / 1.0e3; sigdigits = 4), " μs")
    x < 1.0e9 && return string(round(x / 1.0e6; sigdigits = 4), " ms")
    return string(round(x / 1.0e9; sigdigits = 4), " s")
end
...
println(
    io, "Fresh comparison run for $run_note: SOFA.jl against pyerfa ",
...
println(
    io, "*Julia $(prov["julia_version"]), numpy $(prov["numpy"]), ",
